        ...
        for (std::map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (hasZerocoinSpends) {
                    ...
                    //Priority = (age^6+100000)*amount - gives higher priority to zLiLLis that have been in mempool long
                    //and higher priority to zLiLLis that are large in value
        ...
        if (pblock->nVersion < 7) {
            if (fZerocoinActive) {
                ...
                if (nHeight >= pCheckpointCache.first || pCheckpointCache.second.first != hashBlockLastAccumulated) {
                    //For the period before v2 activation, zLiLLi will be disabled and previous block's checkpoint is all that will be needed
        ...
        if (fProofOfStake) {
            ...
            if (pblock->IsZerocoinStake()) {
                ...
                if (!pwallet->GetZerocoinKey(bnSerial, key)) {
                    LogPrintf("%s: failed to find zLiLLi with serial %s, unable to sign block\n", __func__, bnSerial.GetHex());
                    return NULL;
                }

                //Sign block with the zLiLLi key
                if (!SignBlockWithKey(*pblock, key)) {
                    LogPrintf("%s: Signing new block with zLiLLi key failed \n", __func__);
                    return NULL;
                }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock !=  chainActive.Tip()->GetBlockHash())
            return error("LapislazuliMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        if (pblock->IsZerocoinStake()) {
            pwalletMain->zLiLLiTracker->RemovePending(pblock->vtx[1].GetHash());
            pwalletMain->zLiLLiTracker->ListMints(true, true, true); //update the state
        }
        return error("LapislazuliMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("LapislazuliMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("lapislazuli-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running LapislazuliMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("LapislazuliMiner exception");
    } catch (...) {
        LogPrintf("LapislazuliMiner exception");
    }

    LogPrintf("LapislazuliMiner exiting\n");
}
