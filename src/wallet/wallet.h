...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 LiLLi at the same time
...
enum ZerocoinSpendStatus {
    ZLiLLi_SPEND_OKAY = 0,                            // No error
    ZLiLLi_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZLiLLi_WALLET_LOCKED = 2,                         // Wallet was locked
    ZLiLLi_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZLiLLi_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZLiLLi_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZLiLLi_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZLiLLi_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZLiLLi_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZLiLLi_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZLiLLi_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZLiLLi_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZLiLLi_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZLiLLi_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZLiLLi_SPENT_USED_ZLiLLi = 14,                      // Coin has already been spend
    ZLiLLi_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZLiLLi_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZLiLLiOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    std::string GetUniqueWalletBackupName(bool fzLiLLiAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzLiLLiTracker> zLiLLiTracker;
    ...
    void setZWallet(CzLiLLiWallet* zwallet);
    CzLiLLiWallet* getZWallet();
    ...
    /// Get 10000 LiLLi output and keys which can be used for the Masternode
    ...
    /** zLiLLi reset */
    boost::signals2::signal<void()> NotifyzLiLLiReset;
