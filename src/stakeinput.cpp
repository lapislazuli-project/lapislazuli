...
// The zLiLLi block index is the first appearance of the accumulator checksum that was used in the spend
...
bool CZPivStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zLiLLi staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
{
    //The unique identifier for a zLiLLi is a hash of the serial
...
{
    //Create an output returning the zLiLLi that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZLiLLiOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zLiLLi output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zLiLLi", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZLiLLiOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zLiLLi output", __func__);
...
{
    CzLiLLiTracker* zLiLLiTracker = pwallet->zLiLLiTracker.get();
    CMintMeta meta;
    if (!zLiLLiTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zLiLLiTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
