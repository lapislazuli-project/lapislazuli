...
BOOST_AUTO_TEST_CASE(zerocoin_wrapped_serial_spend_test)
{
    ...
    CzLiLLiWallet *czLiLLiWallet = new CzLiLLiWallet(wallet.strWalletFile);
    ...
    for (unsigned int i = 0; i < TESTS_COINS_TO_ACCUMULATE; i++) {
        ...
        czLiLLiWallet->GenerateDeterministicZLiLLi(denom, coin, dMint, true);
        czLiLLiWallet->UpdateCount();
