...
#include "qt/lapislazuli/forms/ui_masternodeswidget.h"
#include "qt/lapislazuli/qtutils.h"
#include "qt/lapislazuli/mnrow.h"
#include "qt/lapislazuli/mninfodialog.h"

#include "qt/lapislazuli/masternodewizarddialog.h"
...
MasterNodesWidget::MasterNodesWidget(LapislazuliGUI *parent) :
    PWidget(parent),
...
void MasterNodesWidget::onCreateMNClicked(){
    if(verifyWalletUnlocked()) {
        if(walletModel->getBalance() <= CollateralRequired(chainActive.Height())){
            ...
            // kind of like "...node, %i LiLLi requited").arg(qYourCollateralResultVariable),
            // to nicely print required collateral amount.
            inform(tr("Not enough balance to create a masternode")); //, 10,000 LiLLi required."));
            return;
        }
        showHideOp(true);
