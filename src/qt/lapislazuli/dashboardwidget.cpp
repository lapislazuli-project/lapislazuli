...
#include "qt/lapislazuli/forms/ui_dashboardwidget.h"
#include "qt/lapislazuli/sendconfirmdialog.h"
#include "qt/lapislazuli/txrow.h"
...
DashboardWidget::DashboardWidget(LapislazuliGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of LiLLi staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquarePiv, "square-chart-piv");
    setCssProperty(ui->labelSquarezPiv, "square-chart-zLiLLi");
    ...
    ui->labelAmountPiv->setText("0 LiLLi");
    ...
    ui->btnHowTo->setText(tr("How to get LiLLi"));
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isPiv = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZLiLLi;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            zLiLLi = (pair.second != 0) ? pair.second / 100000000 : 0;
        ...
        chartData->valueszPiv.append(zLiLLi);

        int max = std::max(piv, zLiLLi);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("LiLLi");
    set1 = new QBarSet("zLiLLi");
