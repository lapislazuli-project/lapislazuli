...
#include "qt/lapislazuli/send.h"
#include "qt/lapislazuli/forms/ui_send.h"
#include "qt/lapislazuli/addnewcontactdialog.h"
#include "qt/lapislazuli/qtutils.h"
#include "qt/lapislazuli/sendchangeaddressdialog.h"
#include "qt/lapislazuli/optionbutton.h"
#include "qt/lapislazuli/sendconfirmdialog.h"
#include "qt/lapislazuli/myaddressrow.h"
#include "qt/lapislazuli/guitransactionsutils.h"
...
#include "zLiLLicontroldialog.h"

SendWidget::SendWidget(LapislazuliGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (LiLLi)"));
    setCssProperty(ui->labelSubtitle1, "text-subtitle");

    /* Address */
    ui->labelSubtitleAddress->setText(tr("Lapislazuli address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 LiLLi");
...
void SendWidget::onSendClicked(){
    ...
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendPiv ? AskPassphraseDialog::Context::Send_LiLLi : AskPassphraseDialog::Context::Send_zLiLLi, true)){
...
bool SendWidget::sendZpiv(QList<SendCoinsRecipient> recipients){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zLiLLi is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zLiLLi selector if applicable
    ...
    if (walletModel->sendZpiv(
        ...
        inform(tr("zLiLLi transaction sent!"));
        ZPivControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZLiLLi_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zLiLLi require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zLiLLi transaction
        ...
        emit message("zLiLLi transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isLiLLi){
        ...
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            inform(tr("You don't have any zLiLLi in your balance to select."));
        }
    }
}
...
void SendWidget::onLiLLiSelected(bool _isLiLLi){
    isLiLLi = _isLiLLi;
    setCssProperty(coinIcon, _isLiLLi ? "coin-icon-piv" : "coin-icon-piv");
    refreshView();
