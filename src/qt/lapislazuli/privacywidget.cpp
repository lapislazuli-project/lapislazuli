...
#include "qt/lapislazuli/privacywidget.h"
#include "qt/lapislazuli/forms/ui_privacywidget.h"
#include "qt/lapislazuli/qtutils.h"
#include "guiutil.h"
#include "qt/lapislazuli/denomgenerationdialog.h"
#include "qt/lapislazuli/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(LapislazuliGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zLiLLi anonymizes your LiLLi by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zLiLLi or convert back to LiLLi"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 LiLLi ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zLiLLi");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zLiLLi");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zLiLLi");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zLiLLi");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zLiLLi");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zLiLLi");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zLiLLi");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zLiLLi");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zLiLLi Movements"));
    ...
    // Only Convert to LiLLi enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzLiLLi->setTitleClassAndText("btn-title-grey", "Total 0 zLiLLi");
    ui->btnTotalzLiLLi->setSubTitleClassAndText("text-subtitle", "Show denominations of zLiLLi owned.");
    ui->btnTotalzLiLLi->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select LiLLi outputs to mint into zLiLLi.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zLiLLi");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzLiLLi, SIGNAL(clicked()), this, SLOT(onTotalZpivClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zLiLLi");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of LiLLi to mint into zLiLLi"));
    }else{
        btnText = tr("Convert back to LiLLi");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zLiLLi to convert back into LiLLi"));
    }
...
void PrivacyWidget::onTotalZpivClicked(){
    ...
    if(!isVisible){
        ...
        ui->btnTotalzLiLLi->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzLiLLi->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zLiLLi is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zLiLLi, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zLiLLi").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zLiLLi minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZpiv(
        ...
        // Spend succeed
        inform(tr("zLiLLi converted back to LiLLi"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::onCoinControlClicked(){
    if(ui->pushRight->isChecked()) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any LiLLi to select."));
        }
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zLiLLi </b>";
    ...
    ui->btnTotalzLiLLi->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
