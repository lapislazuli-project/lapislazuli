...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered lapislazuli address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > sporkManager.GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 LiLLi.").arg(sporkManager.GetSporkValue(SPORK_5_MAX_VALUE)),
...
static void NotifyzLiLLiReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzLiLLiReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzLiLLiReset.connect(boost::bind(NotifyzLiLLiReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzLiLLiReset.disconnect(boost::bind(NotifyzLiLLiReset, this));
...
bool WalletModel::updateAddressBookPurpose(const QString &addressStr, const std::string& purpose) {
    ...
    if (address.IsStakingAddress())
        return error("Invalid Lapislazuli address, cold staking address");
    CKeyID keyID;
...
bool WalletModel::getKeyId(const CBitcoinAddress& address, CKeyID& keyID) {
    if (!address.IsValid())
        return error("Invalid Lapislazuli address");

    if (!address.GetKeyID(keyID))
        return error("Unable to get KeyID from Lapislazuli address");
...
{
    setMints.clear();
    setMints = pwalletMain->zLiLLiTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
