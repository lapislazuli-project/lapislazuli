...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::LiLLi);
    ...
    if (settings.contains("nAnonymizeLapislazuliAmount") || reset)
        SoftSetArg("-anonymizelapislazuliamount", settings.value("nAnonymizeLapislazuliAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in lapislazuli.cpp during shotdown to also remove the window positions
