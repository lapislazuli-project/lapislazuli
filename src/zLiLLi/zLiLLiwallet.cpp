// Copyright (c) 2017-2019 The Lapislazuli developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "zLiLLiwallet.h"
#include "main.h"
#include "txdb.h"
...
CzLiLLiWallet::CzLiLLiWallet(std::string strWalletFile)
{
...
bool CzLiLLiWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZLiLLiCount(nCountLastUsed);
    else if (!walletdb.ReadZLiLLiCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzLiLLiWallet::Lock()
{
    seedMaster = 0;
}

void CzLiLLiWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzLiLLiWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZLiLLi(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzLiLLiWallet::LoadMintPoolFromDB()
{
...
void CzLiLLiWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzLiLLiWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzLiLLiWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zLiLLiTracker->HasPubcoinHash(pMint.first)) {
...
bool CzLiLLiWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZLiLLi(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    pwalletMain->zLiLLiTracker->Add(dMint, true);
...
void CzLiLLiWallet::SeedToZLiLLi(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzLiLLiWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzLiLLiWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZLiLLiCount(nCountLastUsed);
...
void CzLiLLiWallet::GenerateDeterministicZLiLLi(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzLiLLiWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzLiLLiWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzLiLLiWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
