// Copyright (c) 2018-2020 The Lapislazuli developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zLiLLi/deterministicmint.h>
#include "zLiLLitracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzLiLLiTracker::CzLiLLiTracker(std::string strWalletFile)
{
...
CzLiLLiTracker::~CzLiLLiTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzLiLLiTracker::Init()
{
...
bool CzLiLLiTracker::Archive(CMintMeta& meta)
{
...
bool CzLiLLiTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzLiLLiTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzLiLLiTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzLiLLiTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzLiLLiTracker::GetSerialHashes()
{
...
CAmount CzLiLLiTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzLiLLiTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzLiLLiTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzLiLLiTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzLiLLiTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzLiLLiTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzLiLLiTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzLiLLiTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzLiLLiTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzLiLLiTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzLiLLiTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzLiLLiWallet* zLiLLiWallet)
{
    bool iszLiLLiWalletInitialized = (NULL != zLiLLiWallet);
    CMintMeta meta;
    ...
    if (! iszLiLLiWalletInitialized)
        zLiLLiWallet = new CzLiLLiWallet(strWalletFile);
    meta.isSeedCorrect = zLiLLiWallet->CheckSeed(dMint);
    if (! iszLiLLiWalletInitialized)
        delete zLiLLiWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzLiLLiTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzLiLLiTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzLiLLiTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzLiLLiTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzLiLLiTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzLiLLiTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzLiLLiWallet* zLiLLiWallet = new CzLiLLiWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zLiLLiWallet);
        }
        delete zLiLLiWallet;
        LogPrint("zero", "%s: added %d dzLiLLi from DB\n", __func__, listDeterministicDB.size());
    }
