...
from test_framework.test_framework import LapislazuliTestFramework
...
class ZerocoinSpendTest(LapislazuliTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zLiLLi_bal, piv_bal):
            zLiLLi_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zLiLLi_bal)
            piv_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], piv_bal)
            return zLiLLi_bal, piv_bal
        ...
        zLiLLi_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zLiLLi_balance, 6666)
        ...
        zLiLLi_balance, balance = check_balances(denom_0, zLiLLi_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zLiLLi_balance, balance = check_balances(denom_2, zLiLLi_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zLiLLi_balance, balance = check_balances(denom_3, zLiLLi_balance, balance)
        ...
        assert_raises_rpc_error(-26, "bad-txns-invalid-zLiLLi",
        ...
        _, _ = check_balances(denom_1, zLiLLi_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
