...
The build script we'll be using is contained in the Lapislazuli github repository ([contrib/gitian-build.py](https://github.com/forking-altcoins/lapislazuli/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the Lapislazuli repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/forking-altcoins/Lapislazuli/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, lapislazuli-detached-sigs, and lapislazuli GitHub repos.
