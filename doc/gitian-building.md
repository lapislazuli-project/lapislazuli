...
*Setup instructions for a gitian build of Lapislazuli Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Lapislazuli
...
to the Lapislazuli GitHub release page.
...
- [Building Lapislazuli Core](#building-lapislazuli-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/lapislazuli-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/lapislazuli-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/lapislazuli-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/lapislazuli-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/lapislazuli-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/lapislazuli-win-${VERSION%\.*}-build.assert
```
...
You can also mail the files to Fuzzbawls (fuzzbawls@lapislazuli.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/lapislazuli-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/lapislazuli-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/lapislazuli-osx-*-build.assert
```
