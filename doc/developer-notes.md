...
- [Development guidelines](#development-guidelines)
    - [General Lapislazuli Core](#general-lapislazuli-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Lapislazuli Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play LiLLis (tLiLLi)" on the test network, if you
...
Lapislazuli Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/forking-altcoins/lapislazuli/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_lapislazuli
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_lapislazuli --log_level=test_suite
$ valgrind -v --leak-check=full src/lapislazulid -printtoconsole
```
...
# A coverage report will now be accessible at `./test_lapislazuli.coverage/index.html`.
```
...
- BitcoinMiner : Generates LiLLis (if wallet is enabled).
...
However, in open source software such as Lapislazuli Core, where everyone uses
...
pay attention to for reviewers of Lapislazuli Core code.

General Lapislazuli Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Lapislazuli Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Lapislazuli Core so that they can be integrated
...
file descriptors it will cause problems with Lapislazuli's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:forking-altcoins/Lapislazuli.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `lapislazuli-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `lapislazuli-cli` and the GUI debug console use this table to determine how to
